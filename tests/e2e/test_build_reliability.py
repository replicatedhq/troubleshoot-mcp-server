...
def test_container_build_never_uses_cached_configs(temp_project_dir):
    ...
    try:
        ...
        build_result = subprocess.run([
            runtime, "run", "--rm", "--privileged", "--cap-add=SYS_ADMIN",
            "-v", f"{temp_project_dir}:/work",
            "cgr.dev/chainguard/melange", "build", ".melange.yaml", 
            "--arch=amd64", f"--signing-key=test-key"
        ], capture_output=True, text=True, cwd=temp_project_dir, timeout=60)
        ...
        assert "/invalid/path" in error_output or "No such file" in error_output, (
        ...
        subprocess.run([
            runtime, "rmi", "-f", f"{image_name}:latest"
        ], capture_output=True)
